...
def train():
    ...
    train_dataset = load_data('reviewed_emotion_0311.csv')
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(RE_detail_dataset)):
        print(f"Fold : {fold}")
        ...
        # data load
        train_dataset_fold = RE_detail_dataset.select(train_index)
        val_dataset_fold = RE_detail_dataset.select(val_index)
        
        # Trainer
        trainer_for_detail = Trainer(
        model=model_for_detail,                         # the instantiated 🤗 Transformers model to be trained
        args=training_args_detail,                  # training arguments, defined above
        ...
        models.append(trainer_for_detail)
        # 모델 저장
        # model_for_detail.save_pretrained(f"pts/best_model_main_{fold}")
    
    total_model = IntegratedModel(models)
    torch.save(total_model,"kfold_model.pth")
    
    #push to hub
    API_KEY = "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"
    total_model.push_to_hub(
        "kfold_test_model",
